...
var on = addEventListener,
    ...
    trigger = function(t) { dispatchEvent(new Event(t)); },
    cssRules = function(selectorText) {
      var ss = document.styleSheets, a = [], f = function(s) {
        var r = s.cssRules, i;
        for (i = 0; i < r.length; i++) {
          if (r[i] instanceof CSSMediaRule &&
              matchMedia(r[i].conditionText).matches)
            f(r[i]);
          else if (r[i] instanceof CSSStyleRule &&
      ...
      if (!e)
        ...
        switch (e.dataset.scrollBehavior ? e.dataset.scrollBehavior
                                         : 'default') {
        case 'default':
        default:
        ...
        case 'previous':
          if (e.previousElementSibling)
            y = e.previousElementSibling.offsetTop +
                e.previousElementSibling.offsetHeight + offset;
          else
            y = e.offsetTop + offset;
          break;
        }
...
if (client.mobile) {
  (function() {
    if (client.flags.lsdUnits) {
      ...
      document.documentElement.style.setProperty('--background-height',
                                                 '100lvh');
    } else {
      var f = function() {
        ...
        document.documentElement.style.setProperty(
            '--background-height', window.innerHeight + 250 + 'px');
      };
      on('load', f);
      on('resize', f);
      on('orientationchange',
         function() { setTimeout(function() { f(); }, 100); });
    }
  })();
}
if (client.os == 'android') {
  ...
  if (client.osVersion <= 11)
    (function() {
      ...
      on('focus', function(event) { $body.classList.add('ios-focus-fix'); },
         true);
      on('blur', function(event) { $body.classList.remove('ios-focus-fix'); },
         true);
    })();
  $body.classList.add('is-touch');
}
...
elements.forEach(function(e) {
  ...
  if ('reorderBreakpoint' in e.dataset && e.dataset.reorderBreakpoint in
                                              breakpoints)
...
var scrollEvents = {
  ...
  add : function(o) {
    this.items.push({
      element : o.element,
      triggerElement : 'triggerElement' in o && o.triggerElement
                           ? o.triggerElement
                           : o.element,
  ...
  handler : function() {
    ...
    scrollEvents.items.forEach(function(item) {
      ...
      if (item.initialState !== null) {
        ...
        case 1:
        default:
          state = bottom > elementTop - item.offset &&
...
var onvisible = {
  effects : {
    ...
    'slide-left' : {
      ...
      rewind : function() { this.style.transform = 'translateX(100vw)'; },
      play : function() { this.style.transform = 'none'; },
    },
    'slide-right' : {
      transition : function(speed, delay) {
        return ('transform ' + speed + 's ease' +
    ...
    'flip-left' : {
      ...
      rewind : function(intensity, alt) {
        this.style.opacity = 0;
        this.style.transformOrigin = '50% 50%';
        this.style.transform = 'perspective(1000px) rotateY(' +
                               (alt ? 45 : 15) * intensity + 'deg)';
      },
      play : function() {
        this.style.opacity = 1;
    ...
    'fade-in-background' : {
      ...
      play : function() {
        this.style.setProperty('--onvisible-background-color',
                               'rgba(0,0,0,0.001)');
      },
    },
    'zoom-in-image' : {
      ...
      rewind : function() { this.style.transform = 'scale(1)'; },
          ...
          enter = function(staggerDelay = 0) {
            ...
            if (!effect.custom) {
              ...
              _this.style.transition =
                  effect.transition(speed, delay + staggerDelay);
            } else
